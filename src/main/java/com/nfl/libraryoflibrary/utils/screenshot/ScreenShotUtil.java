package com.nfl.libraryoflibrary.utils.screenshot;
...
//            method = Class.forName(surfaceClassName).getDeclaredMethod("screenshot", new Class[]{Integer.TYPE, Integer.TYPE});
            method = Class.forName(surfaceClassName).getDeclaredMethod("screenshot", int.class, int.class);
        } else {
            method = Class.forName(surfaceClassName).getDeclaredMethod("screenshot", Rect.class, int.class, int.class, int.class);
        }
        method.setAccessible(true);
        if (Build.VERSION.SDK_INT < 28) {
            b = (Bitmap) method.invoke(null, size.x, size.y);
        } else {
            b = (Bitmap) method.invoke(null, new Rect(0, 0, size.x, size.y), size.x, size.y, 0);
        }
